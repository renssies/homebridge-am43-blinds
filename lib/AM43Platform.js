...
class AM43Platform {
    ...
    static CONFIG_KEY_SCANNING_TIMEOUT = "scanning_timeout"
    static CONFIG_KEY_POLL_INTERVAL = "poll_interval"
    static CONFIG_KEY_HAP_INTERACTION_TIMEOUT = "hap_interaction_timeout"
    static CONFIG_KEY_ALLOWED_DEVICES = "allowed_devices"

    static DEFAULT_HAP_INTERACTION_TIMEOUT = 1.5 * 60 // The minimum amount of time since HAP has interacted with the device before it should disconnect. In seconds
    static DEFAULT_POLL_INTERVAL = 5 * 60 // The time between polling requests for the position, battery and light sensor. In seconds
    static DEFAULT_SCANNING_TIMEOUT = 10 // The time for which the plugin should scan for devices during launch. In seconds

    static POSITION_UPDATE_INTERVAL = 2 * 60 // The minimum time between the request for position updates. In seconds
    static MISSING_DEVICES_SCANNING_TIMEOUT = 5 // The time for which the plugin should scan for devices when it is missing a device. In seconds
    static HAP_NO_INTERACTION_GRACE_PERIOD = 5 // The grace period that is applied when the HAP interaction timeout has been reached. This is to give HAP some time to interact with the device before disconnection. In seconds
    static MINIMUM_POLL_INTERVAL = 5 // The minimum required poll interval. In seconds.
    ...
    constructor(log, config, api) {
        ...
        let configuredAllowedDevicesList = this.configJSON[AM43Platform.CONFIG_KEY_ALLOWED_DEVICES]
        if (configuredAllowedDevicesList !== undefined) {
            if (configuredAllowedDevicesList == null) {
                ...
                this.log.error(`The config.json defines '${AM43Platform.CONFIG_KEY_ALLOWED_DEVICES}' list but it seems to be an invalid format. The list should be an array, example: ['MAC1', 'MAC2']`)
                this.allowedDevices = []
            }
        } else {
            this.log.debug(`No ${AM43Platform.CONFIG_KEY_ALLOWED_DEVICES} field found. Ignoring all devices`)
            this.allowedDevices = []
        }

        if (this.configJSON[AM43Platform.CONFIG_KEY_HAP_INTERACTION_TIMEOUT] != undefined && this.configJSON[AM43Platform.CONFIG_KEY_HAP_INTERACTION_TIMEOUT] <= 0) {
        ...
        if (this.configJSON[AM43Platform.CONFIG_KEY_POLL_INTERVAL] != undefined && this.configJSON[AM43Platform.CONFIG_KEY_POLL_INTERVAL] < AM43Platform.MINIMUM_POLL_INTERVAL) {
            this.log.warn(`Polling for devices is disabled due too a low poll interval. This might cause an incorrect state in HomeKit apps. Polling requires a value of ${AM43Platform.MINIMUM_POLL_INTERVAL} (seconds) or higher.`)
        }
    ...
    didFinishedLaunching() {
        ...
        noble.on('discover', (peripheral) => {
            ...
            if (this.allowedDevices != null) {
                ...
                if (!this.allowedDevices.includes(deviceIdentifier)) {
                    this.log.warn(`Device ${device.description} is not found on the '${AM43Platform.CONFIG_KEY_ALLOWED_DEVICES}' array in config.json and is ignored.`)
                    ...
                    this.log.warn(`Or set '${AM43Platform.CONFIG_KEY_ALLOWED_DEVICES}' to 'null' to allow all devices. Setting 'null' is not recommended!`)
                    return
                }
            }
        ...
        let scanningTimeout = this.configJSON[AM43Platform.CONFIG_KEY_SCANNING_TIMEOUT] ? this.configJSON[AM43Platform.CONFIG_KEY_SCANNING_TIMEOUT] : AM43Platform.DEFAULT_SCANNING_TIMEOUT
    ...
    configurePropertiesOnAccessory(accessory) {
        ...
        accessory.hapInteractionTimeout = this.configJSON[AM43Platform.CONFIG_KEY_HAP_INTERACTION_TIMEOUT] != undefined ? this.configJSON[AM43Platform.CONFIG_KEY_HAP_INTERACTION_TIMEOUT] : AM43Platform.DEFAULT_HAP_INTERACTION_TIMEOUT
        ...
        accessory.checkForHAPInteractionTimeout = function() {
            if (!this.am43device.isConnected) { return }
            if (this.hapInteractionTimeout > 0 && this.secondsSinceLastHAPInteraction() >= this.hapInteractionTimeout) {
                this.log.debug("HAP interaction timeout reached, starting " + AM43Platform.HAP_NO_INTERACTION_GRACE_PERIOD + " second grace period")
                ...
                }, AM43Platform.HAP_NO_INTERACTION_GRACE_PERIOD * 1000)
            }
        }.bind(accessory)

        accessory.scanForMissingDevices = function() {
            this.startScanningForDevices(AM43Platform.MISSING_DEVICES_SCANNING_TIMEOUT)
    ...
    configureDeviceOnAccessory(accessory, device) {
        ...
        let pollInterval = this.configJSON[AM43Platform.CONFIG_KEY_POLL_INTERVAL] != undefined ? this.configJSON[AM43Platform.CONFIG_KEY_POLL_INTERVAL] : AM43Platform.DEFAULT_POLL_INTERVAL
        if (pollInterval >= AM43Platform.MINIMUM_POLL_INTERVAL) {
            setTimeout(() => {
    ...
    configureWindowCoveringServiceOnAccessory(accessory) {
        ...
        service.getCharacteristic(this.Characteristic.CurrentPosition)
            .on('get', function(callback) {
                ...
                if (this.secondsSinceLastPositionUpdate() > AM43Platform.POSITION_UPDATE_INTERVAL || !this.am43device.isConnected) {
