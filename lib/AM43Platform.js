...
class AM43Platform {
  constructor(log, config, api) {
    ...
    let configuredAllowedDevicesList =
      this.configJSON[StaticVariables.CONFIG_KEY_ALLOWED_DEVICES]
  ...
  startScanningForDevices(timeout) {
    ...
    noble.startScanning([DeviceValues.AM43_SERVICE_ID], false, (error) => {
  ...
  configureWindowCoveringServiceOnAccessory(accessory) {
    ...
    service
      ...
      .on("get", function (callback) {
        ...
        accessory.lastHAPInteraction = Date.now()
        accessory.log.debug(
          "Reporting direction: " + accessory.am43device.direction
        )
