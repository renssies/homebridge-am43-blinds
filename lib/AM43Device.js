...
class AM43Device extends EventEmitter {
    ...
    setBlindsControlCharacteristic(characteristic) {
        ...
        this.blindsControlCharacteristic.on('data', (data) => {
            ...
            if (data[1] == StaticVariables.AM43_COMMAND_ID_GET_POSITION) {
            ...
            } else if (data[1] == StaticVariables.AM43_COMMAND_ID_GET_LIGHTSENSOR) {
            ...
            } else if (data[1] == StaticVariables.AM43_COMMAND_ID_GET_BATTERYSTATUS) {
            ...
            } else if (data[1] == StaticVariables.AM43_NOTIFY_POSITION) {
                this.debugLog("Position notify received");
                let percentage = parseInt(dataArray[4])
                this.debugLog(`Closed Percentage ${percentage}`)
                this.position = percentage
                this.emit('position', this.position)
            } else if (data[1] == StaticVariables.AM43_COMMAND_ID_SET_MOVE) {
            ...
            } else if (data[1] == StaticVariables.AM43_COMMAND_ID_SET_POSITION) {
    ...
    async connectAsync() {
        ...
        if (this.discoveringPromise == null) {
            this.discoveringPromise = this.peripheral.discoverSomeServicesAndCharacteristicsAsync([StaticVariables.AM43_SERVICE_ID], [StaticVariables.AM43_CHARACTERISTIC_ID])
        }
        let {characteristics} = await this.discoveringPromise
    ...
    async setPositionAsync(position, trackPosition) {
        ...
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_SET_POSITION, [position])
    ...
    async openAsync() {
        ...
        this.direction = 1
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_SET_MOVE, [AM43_MOVE_OPEN])
        this.emit("direction", this.direction)
        this.emit("targetPosition", this.targetPosition)
    }
    ...
    async closeAsync() {
        ...
        this.direction = 0
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_SET_MOVE, [AM43_MOVE_CLOSE])
        this.emit("direction", this.direction)
        this.emit("targetPosition", this.targetPosition)
    }
    ...
    async stopAsync() {
        this.targetPosition = null
        this.direction = 2
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_SET_MOVE, [AM43_MOVE_STOP])
        this.emit("direction", this.direction)
        this.emit("targetPosition", this.targetPosition)
    }
    
    async updatePositionAsync() {
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_GET_POSITION, [0x1])
    }

    async updateBatteryStatusAsync() {
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_GET_BATTERYSTATUS, [0x1])
    }

    async updateLightSensorAsync() {
        await this.sendCommandAsync(StaticVariables.AM43_COMMAND_ID_GET_LIGHTSENSOR, [0x1])
    }
    ...
    async sendCommandAsync(commandID, data) {
        ...
        var startPackage = StaticVariables.AM43_COMMAND_PREFIX
